...
/**
 * Image buffer for mutable creation of an image, use toImage when finished to convert to a standard immutable
 * image
 */
class ImageBuffer[@specialized(Int, Float, Double) Pixel: ClassTag](
  ...
  private def rawTransform(f: Pixel => Pixel): Unit =
    (0 until domain.length).foreach(i => data(i) = f(data(i)))
  ...
  // / in-place transformation
  def transformWithIndex(f: (Int, Int, Pixel) => Pixel): Unit = {
    copyOnWrite((0 until domain.length).foreach(i => data(i) = f(domain.x(i), domain.y(i), data(i))))
  }
  ...
  /**
   * *unsafe* make a read-only copy which is an unsafe view of this buffer, changes with every mutable
   * operation!
   */
...
object ImageBuffer {
  ...
  def apply[@specialized(Int, Float, Double) Pixel: ClassTag](
      width: Int,
      height: Int,
      data: Array[Pixel]): ImageBuffer[Pixel] = {
  ...
  def tabulate[Pixel: ClassTag](width: Int, height: Int)(f: (Int, Int) => Pixel): ImageBuffer[Pixel] = {
    ...
    buffer.transformWithIndex((x: Int, y: Int, _: Pixel) => f(x, y))
    buffer
  }
